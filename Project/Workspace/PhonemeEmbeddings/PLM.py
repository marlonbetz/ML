...
dim_embedding_word = 200
dim_embedding_language = 50
dim_embedding_info_bagOfPhonemes = 100
dim_embedding_info_geo = 50
...
embedded_phoneme = Embedding(n_symbols+1, dim_embedding_phono, input_length=maxlen,mask_zero=True)(input_phono)
...
concat_info = merge([embedded_language,embedded_info_bagOfPhonemes,embedded_info_geo],mode="concat")

#hadamard product of sequence encoding and info
hadamard = merge([embedded_word,concat_info],mode="mul")


#fully connected layer
concat = Dense(1000,activation="relu")(hadamard)
...
model.fit([data_train_phono_X,data_train_language,data_train_info_bagOfPhonemes,data_train_info_geo],data_train_phono_y,batch_size=100,nb_epoch=14)
...
with codecs.open("phoneme_embeddings_plm.csv","w") as f:
    for phoneme in phoneme_indices:
        #print(phoneme,[phoneme_indices[phoneme]])
        embedding = phoneme_embedder.predict(np.array([phoneme_indices[phoneme]]+(maxlen-1)*[0]).reshape(1,maxlen))[0,0]
        f.write("\""+phoneme+"\"")
...
language_embedder = Model(input_language,embedded_language)
